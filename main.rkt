...
(define-struct state (Bird pipes variables abilities score) #:transparent)

(define b (BIRD bird-initial-y 0)) ;initial bird

(define v (variables initial-gravity initial-momentum initial-scroll-speed)) ;initial variables

(define a (abilities '() '())) ;initial abilities

(define p (pipe (+ added-number (random random-threshold)) scene-width)) ;initial pipe

(define s (state  b (list p) v a 0)) ;initial state
...
(define (get-bird state1)
  (state-Bird state1))
  
(define (get-bird-y bird1)
  (BIRD-y bird1))
...
(define (next-state-bird bird1 gravity)
  (struct-copy BIRD bird1 [y (+ (get-bird-y bird1) (get-bird-v-y bird1))] [v-y (+ gravity (get-bird-v-y bird1))]))

;TODO 4
; După aceasta, implementati un getter care extrage din structura voastră
; viteza pe y a păsării.
(define (get-bird-v-y bird1)
  (BIRD-v-y bird1))
...
(define (next-state-bird-onspace bird1 momentum)
  (struct-copy BIRD bird1 [v-y (- momentum)]))
...
(define (change current-state pressed-key)
 ;(struct-copy state current-state [bird (state-bird current-state)] [pipe (state-pipe current-state)] [variables (state-pipe current-state)] [abilities (state-abilities current-state)] [score (state-score current-state)]))
  (if (key=? pressed-key " ")
  (struct-copy state current-state [Bird (next-state-bird-onspace (get-bird current-state) initial-momentum)])
  current-state))
...
(define (move-pipes pipes scroll-speed)
  (map (λ(pipeX)(struct-copy pipe pipeX [x (- (get-pipe-x pipeX) scroll-speed)])) pipes))
...
(define (clean-pipes pipes)
 (filter (λ(pipeX)(if (<= (+ pipe-width (get-pipe-x pipeX)) 0) #f #t)) pipes))
...
(define (add-more-pipes pipes)
  (if (>= (length pipes) no-pipes)
      pipes
      (append pipes (list (struct-copy pipe (car (reverse pipes))
                                          [gap-y (+ added-number (random random-threshold))]
                                          [x (+ pipe-width pipe-gap (get-pipe-x (car (reverse pipes))))])))))
...
(define (check-ground-collision bird1)
 (if (>= (+ (get-bird-y bird1) bird-height) ground-y)
     #t
     #f))
...
(define (invalid-state? state1)
  (or (check-ground-collision (get-bird state1)) (check-pipe-collisions (get-bird state1) (get-pipes state1))))
...
(define (check-pipe-collisions bird1 pipes)
  (if (null? pipes)
      #f
      ;collision with lower pipe
      (or (check-collision-rectangles (make-posn bird-x (get-bird-y bird1))
                                      (make-posn (+ bird-x bird-width) (+ (get-bird-y bird1) bird-height))
                                      (make-posn (get-pipe-x (car pipes)) (+ pipe-self-gap (pipe-gap-y (car pipes))))
                                      (make-posn (+ (get-pipe-x (car pipes)) pipe-width) ground-y))
          ;collsion with upper pipe
          (check-collision-rectangles (make-posn bird-x (BIRD-y bird1))
                                      (make-posn (+ bird-x bird-width) (+ (BIRD-y bird1) bird-height))
                                      (make-posn (get-pipe-x (car pipes)) 0)
                                      (make-posn (+ (get-pipe-x (car pipes)) pipe-width) (pipe-gap-y (car pipes))))
          ;check collision with next pipe
          (check-pipe-collisions bird1 (cdr pipes)))))
...
(define (next-state state1)
  (struct-copy state state1
               [Bird (next-state-bird (get-bird state1) initial-gravity)]
               [pipes (next-state-pipes (get-pipes state1) initial-scroll-speed)]
               [score (+ 0.1 (get-score state1))]))
...
(define (draw-frame state1)
  (place-image bird-image (+ bird-x (quotient bird-width 2)) (+ (get-bird-y (get-bird state1)) (quotient bird-height 2))
               (place-image ground-image (quotient scene-width 2) (- scene-height (quotient ground-height 2))
                            (place-image (score-to-image (get-score state1)) text-x text-y
                                         (place-pipes (get-pipes state1) initial-scene)))))
...
(define (place-pipes pipes scene)
	(if (null? pipes)
            ...
            (place-image (rectangle pipe-width pipe-height "solid" "green")
                         ...
                         (- (pipe-gap-y (car pipes)) (quotient pipe-height 2))
            (place-image (rectangle pipe-width pipe-height "solid" "green")
                         (+ (get-pipe-x (car pipes)) (quotient pipe-width 2))
                         (+ (pipe-gap-y (car pipes)) pipe-self-gap (quotient pipe-height 2))
